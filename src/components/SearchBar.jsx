...
async function fetchBooks(query) {
	return (
		await axiosClient.get("/books/search", { params: { search: query } })
	).data;
}
...
export default function SearchBar() {
	...
	return (
		<div
			ref={searchRef}
			className="relative box-border flex bg-white border-2 border-slate-500 md:min-w-96 mt-4 md:mt-0"
		>
			<input
			...
			<div
				className={`${
					isQueryEnabled ? "" : "hidden"
				} bg-white box-border z-50 shadow-lg absolute top-full left-0 w-full flex flex-col gap-1 p-2 max-h-80 overflow-auto`}
			>
				{results.isFetching ? (
