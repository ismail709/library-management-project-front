...
async function fetchRecentBooks(pageParam) {
    return (await axiosClient.get("/books/recent",{params:{page:pageParam}})).data;
}


export default function RecentPage(){
    const recentBooksQuery = useInfiniteQuery({
        queryKey: ["recentpage"],
        queryFn: ({pageParam}) => fetchRecentBooks(pageParam),
        initialPageParam:1,
    ...
    return <div className="flex flex-col justify-center items-center gap-4 py-8 w-full max-w-full">
                        ...
                        <button 
                            ...
                            onClick={() => recentBooksQuery.fetchNextPage()}
                            disabled={!recentBooksQuery.hasNextPage || recentBooksQuery.isFetchingNextPage}
                        >
                            {recentBooksQuery.isFetchingNextPage? 'Loading more...' : recentBooksQuery.hasNextPage ? 'Load more' : 'Nothing more to load'}
                        </button>
                        }
                    </div>
}