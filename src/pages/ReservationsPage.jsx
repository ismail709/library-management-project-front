...
function fetchReservations(pageParam) {
	// return axiosClient.get("/users/reservations");
	return axiosClient.get("/users/reservations", {
...
export default function ReservationsPage() {
	...
	return (
		<div className="flex flex-col justify-center items-center gap-4 py-8 px-4 lg:px-0 w-full max-w-full">
				...
				<table className="w-full">
					<thead>
						<tr className="border-b border-slate-500 text-left text-slate-600">
							<th className="px-4 py-2 min-w-screen md:min-w-60">Book</th>
					...
					<tbody>
						{reservationsQuery.isSuccess &&
							reservationsQuery.data.pages.map((page) => {
								return page.data.data.map((reservations, i) => (
									<tr
										key={i}
