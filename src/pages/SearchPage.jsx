...
async function fetchBooks(query, pageParam) {
	return (
		await axiosClient.get("/books/search", {
			params: { search: query, limit: false, page: pageParam },
		})
	).data;
}
...
export default function SearchPage() {
	...
	const results = useInfiniteQuery({
		...
		queryFn: ({ pageParam }) =>
			fetchBooks(searchParams.get("s"), pageParam),
		initialPageParam: 1,
	...
	return (
		<div className="flex flex-col justify-center items-center gap-4 py-8 w-full max-w-full">
			<h1 className="text-2xl font-bold text-slate-500 capitalize">
			...
			<div className="grid place-items-center grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 w-full max-w-full">
				...
				{results.isSuccess
					? results.data.pages[0].data.length == 0 && (
